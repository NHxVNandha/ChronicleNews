...
import { articles, categories, heroImage } from '../../data';
      ...
      <main className="container-page space-y-16 py-12">
        <section className="grid overflow-hidden rounded-xl bg-white editorial-shadow lg:grid-cols-12">
          ...
          <div className="flex flex-col justify-center p-8 lg:col-span-5 lg:p-12">
            ...
            <h1 className="font-display text-4xl font-bold leading-tight text-primary lg:text-5xl">The Architecture of Silence: Rebuilding the Digital Commons</h1>
        ...
        <section className="grid gap-10 border-t border-slate-200 pt-16 lg:grid-cols-12">
          ...
          <div className="grid gap-6 lg:col-span-8 md:grid-cols-2">
            {categories.map((category) => (
              <div key={category.name} className="rounded-xl bg-white p-6 soft-shadow">
                <div className={`mb-5 h-2 w-14 rounded-full ${category.tone}`} />
                ...
                <p className="mt-5 text-sm font-bold uppercase tracking-wider text-secondary">{category.count} Articles</p>
              </div>
            ))}
          </div>
        </section>

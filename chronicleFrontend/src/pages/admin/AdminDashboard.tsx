              ...
              <AdminPanel title="Today" description="Live status" tone="subtle" padding="md">
                <div className="space-y-3">
                  {snapshotData.map((item) => (
                    <div key={item.label} className="flex items-center gap-3 rounded-xl border border-slate-100 bg-white p-3 shadow-sm">
            ...
            <AdminPanel>
              ...
              <div className="mt-6 grid gap-4 md:grid-cols-4">
                {pipelineStages.map((stage) => (
                  <div key={stage.label} className={`rounded-xl border ${stage.bg} border-slate-100 p-4 transition hover:border-slate-200 hover:bg-white`}>
              ...
              <AdminPanel action={<Link className="text-sm font-bold text-secondary hover:underline" to="/admin/content">Open editorial</Link>}>
                ...
                <div className="mt-4 space-y-3">
                  {scheduleSnapshot.map((item) => (
                    <div key={`${item.time}-${item.title}`} className="flex items-start gap-3 rounded-xl border border-slate-100 bg-slate-50 p-3">
